...
class DevisController extends Controller
{
        ...
        try {
            ...
            foreach ($request->lignes as $ligne) {
                if ($request->product_mode === 'new') {
                    ...
                    if (empty($designation) || $prix_unitaire <= 0)
                        continue;
            ...
            $devis = Document::create([
                'type' => 'devis',
                'numero' => Document::genererNumero('devis'),
                'client_id' => $client->id,
    ...
    public function transformerEnFacture($id)
    {
        ...
        // Vérifier si une facture existe déjà pour ce devis (même client, même montant)
        $factureExistante = Document::where('type', 'facture')
            ->where('client_id', $devis->client_id)
            ->where('total_ttc', $devis->total_ttc)
            ->where('objet', $devis->objet)
            ->exists();
        ...
        // On crée la facture en copiant le devis avec un nouveau numéro
